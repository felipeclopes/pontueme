class AlterViewForCardBusinessPoints < ActiveRecord::Migration
  def up
    ...
    ActiveRecord::Base.connection.execute <<-SQL
      CREATE VIEW card_points AS
        SELECT  
          ...
          COALESCE((SELECT SUM(co.points) FROM coupons co JOIN benefits be ON be.id = co.benefit_id WHERE co.card_id = ca.id AND be.business_id = b.id), 0) AS benefit_points,
          SUM(c.points) - COALESCE((SELECT SUM(co.points) FROM coupons co JOIN benefits be ON be.id = co.benefit_id WHERE co.card_id = ca.id AND be.business_id = b.id),0) AS points
        FROM cards ca
